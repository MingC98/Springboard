...
SELECT m.firstname||' '||m.surname AS member_name , m1.surname||','||m1.firstname AS recommendedby
FROM Members m JOIN Members m1
WHERE m.recommendedby = m1.memid
ORDER BY m1.surname

/* Q12: Find the facilities with their usage by member, but not guests */

SELECT f.name AS facility, COUNT(b.memid)
FROM Bookings b
JOIN Facilities f ON b.facid = f.facid
WHERE b.memid != 0
GROUP BY facility
...
SELECT f.name AS facility, EXTRACT(MONTH FROM b.starttime) AS month ,COUNT(b.memid)
FROM Bookings b
JOIN Facilities f ON b.facid = f.facid
WHERE b.memid != 0
